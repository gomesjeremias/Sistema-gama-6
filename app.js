    ...
    vendasProdutoChart = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{
                label: 'Total Vendido',
                data: chartData,
                backgroundColor: ['#2563eb', '#f97316', '#16a34a', '#facc15', '#9333ea', '#db2777'],
                hoverOffset: 4
            }]
        },
        options: {
...
function setupSaleForm() {
    ...
    const calculateTotal = () => {
        ...
        if (productId && quantity > 0) {
            ...
            if (product) {
                totalInput.value = (product.preco * quantity).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' });
            }
        } else {
...
function populateSaleFormForNew() {
    ...
    db.getAll('clientes').forEach(c => clientSelect.innerHTML += `<option value="${c.id}">${c.nome}</option>`);

    const productSelect = document.getElementById('sale-product');
    productSelect.innerHTML = '<option disabled selected>Selecione um produto</option>';
    db.getAll('produtos').forEach(p => productSelect.innerHTML += `<option value="${p.id}">${p.nome}</option>`);
}
...
function setupEventListeners() {
    ...
    document.querySelector('button[onclick="produto_modal.showModal()"]').addEventListener('click', populateProductFormForNew);
    document.querySelector('button[onclick="fornecedor_modal.showModal()"]').addEventListener('click', populateSupplierFormForNew);
    document.querySelector('button[onclick="sale_modal.showModal()"]').addEventListener('click', populateSaleFormForNew);
